...
const HTML_SRC = join(ROOT, "scripts", "promo-newtab.html");
const HTML_OUT = join(ROOT, "scripts", ".promo-newtab.out.html");
...
function downloadCovers(albums) {
  ...
  for (const [i, album] of albums.entries()) {
    const dest = join(COVER_DIR, `${String(i).padStart(2, "0")}.jpg`);
    if (!existsSync(dest)) {
      const r = spawnSync(
        "curl",
...
function jpegFromPng(pngPath, dest, w, h) {
  ...
  if (py.status !== 0) {
    throw new Error(py.stderr || r.stderr || "jpeg convert failed");
  }
}

const files = downloadCovers(pickAlbums(NEED));
if (files.length < 8) {
...
const pngPath = join(COVER_DIR, "shot.png");
await page.screenshot({ path: pngPath, type: "png" });
await browser.close();

jpegFromPng(pngPath, join(ROOT, "public", "card.jpg"), 1280, 640);
jpegFromPng(pngPath, join(ROOT, "public", "newtab.jpg"), 1280, 640);
jpegFromPng(pngPath, join(ROOT, "public", "og.jpg"), 1200, 630);
console.log("wrote public/card.jpg, public/newtab.jpg, public/og.jpg");
